...
def bayesian_optimization_min_uncertainty_battery_simulated_annealing(x):
    ...
    saplan = UncertaintyBatterySimulatedAnnealingSolver(rplan.state, mapper, nb_drone, penalizer=x[0,3])
    hist = []
    for i in range(5):
        rplan.solve()
...
domain =[{'name': 'tmax', 'type': 'continuous', 'domain': (0.01, 1)},
         ...
         {'name': 'ite', 'type': 'continuous', 'domain': (0.025, 1)},
         {'name': 'pen', 'type': 'continuous', 'domain': (0.1, 1)}]
