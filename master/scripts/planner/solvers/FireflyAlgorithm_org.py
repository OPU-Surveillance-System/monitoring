...
def alphaStep6(a, t, step, schedule):
    # alpha = math.ceil(np.random.exponential(delta, 1)*32)
    if schedule == "linear":
        alpha = len(a) - (t//step)
    elif schedule == "sqrt":
        alpha = int(len(a) - (t//step)**(1/2))
    # if alpha > len(a):
    #     alpha = len(a)
    if alpha < 2:
        alpha = 2
...
def fireflyAlgorithm(z, **kwargs):
    ...
    while t < kwargs['i']:
                ...
                if j != i:
                    if swarm[j].luminosity < swarm[i].luminosity:
                        ...
                        if kwargs['v'] == 1:
                            ...
                            c = alphaStep6(c, t, kwargs['s'], kwargs['sch'])
                        swarm[i].update(c)
        # swarm = sorted(swarm, key = lambda ff: ff.luminosity)
        ...
        if t % 100 == 0:
            ...
            # with open("alpha6/new/alpha6_step{}".format(kwargs['s']), "a") as f:
...
if __name__ == "__main__":
    ...
    if not os.path.exists("plots"):
        os.makedirs("plots")
    bestFirefly = fireflyAlgorithm(0, d=args.d, i=args.i, g=args.g, a=args.a, f=args.f, e=args.e, v=args.v, p=args.p, s=args.s, sch=args.sch)
