...
def fireflyAlgorithm(z, **kwargs):
    ...
    while t < kwargs['i']:
        for i in range(n):
            for j in range(n):
                if j != i:
                    if swarm[i].luminosity < swarm[j].luminosity:
                        c = betaStep(swarm[i].x, swarm[j].x, kwargs['g'])
        ...
        if t % 100 == 0:
            if kwargs['p'] == 1:
                ...
                with open("sort/reverse_run10", "a") as f:
...
if __name__ == "__main__":
    ...
    # bestFirefly = fireflyAlgorithm(1, d=args.d, i=args.i, g=args.g, a=args.a, f=args.f, e=args.e, v=args.v, p=args.p, s=args.s, sch=args.sch)
    #     if args.p == 1:
    ...
    with open("sort/reverse_run10", "w") as f:
        if args.p == 1:
            print("cleaning previous results")
    best=[]
    each_best=[]
    for i in range(args.n):
        bestFirefly, e = fireflyAlgorithm(i, d=args.d, i=args.i, g=args.g, a=args.a, f=args.f, e=args.e, v=args.v, p=args.p, s=args.s, sch=args.sch)
    ...
    with open("sort/reverse_run10.pickle", "wb") as f:
