...
def displayPlot(tab,n):
    plt.plot(tab[0],tab[1], label="not_sorted")
    plt.plot(tab[0],tab[2], label="sorted")
...
def fireflyAlgorithm(z, **kwargs):
    ...
    sss = sorted(swarm, key = lambda ff: ff.luminosity)
    ...
    while t < kwargs['i']:
        ...
        if all([s.luminosity == swarm[0].luminosity for s in swarm[1:]]): #true:全要素が等しい
            if kwargs['p'] == 1:
                print("*** swarm blocked ***")
        ...
        if t % 100 == 0:
            if kwargs['p'] == 1:
                ...
                # with open("sort/firefly_notsort", "a") as f:
...
if __name__ == "__main__":
    ...
    # with open("sort/firefly_notsort", "w") as f:
    #     if args.p == 1:
    #         print("cleaning previous results")
    # best=[]
    # each_best=[]
    ...
    #     f.write("best_mean: {}\n\n".format(mean(best)))
    #
    # each_mean = [[i for i in range(args.i)], [0 for j in range(args.i)]]
    # for i in range(args.i):
    #     for j in range(args.n):
    #         each_mean[1][i] += each_best[j][i]
    #     each_mean[1][i] /= args.n
    #
    # with open("sort/sort_mean100.pickle", "rb") as f:
    #     sort_mean = pickle.load(f)
    # each_mean.append(sort_mean)
    # displayPlot(each_mean, "compare_sort_ite100")
