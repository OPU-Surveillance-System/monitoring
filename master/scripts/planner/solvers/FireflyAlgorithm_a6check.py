...
def fireflyAlgorithm(z, **kwargs):
    ...
    while t < kwargs['i']:
        ...
        if t % 100 == 0:
            ...
            if kwargs['c'] == 1:
                with open("alpha6/alpha5to6", "a") as f:
                    f.write("{}\n".format(bestFirefly.luminosity))
            else:
                with open("alpha6/alpha5", "a") as f:
                    f.write("{}\n".format(bestFirefly.luminosity))
            tab[0].append(t+1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-dl", type = float, default = 0.085, help = "firefly algorithm delta")
    ...
    parser.add_argument("-c", type = int, default = 0, help = "change/not_change alphaStep5 to 6 when swarm block")
    ...
    with open("alpha6/alpha5to6", "w") as f:
        if args.p == 1:
            print("cleaning previous results")
    with open("alpha6/alpha5", "w") as f:
        if args.p == 1:
            print("cleaning previous results")
    best_c0=[]
    best_c1=[]
    for i in range(args.n):
        bestFirefly_c0 = fireflyAlgorithm(i, dl=args.dl, d=args.d, i=args.i, g=args.g, a=args.a, f=args.f, e=args.e, v=args.v, c=0, p=args.p, s=args.s, sch=args.sch)
        bestFirefly_c1 = fireflyAlgorithm(i, dl=args.dl, d=args.d, i=args.i, g=args.g, a=args.a, f=args.f, e=args.e, v=args.v, c=1, p=args.p, s=args.s, sch=args.sch)
        best_c0.append(bestFirefly_c0.luminosity)
        best_c1.append(bestFirefly_c1.luminosity)
        # if args.p == 1:
        #     print("Best firefly path: ", bestFirefly.x)
        #     print("Best firefly luminosity: ", bestFirefly.luminosity)
    c0mean = np.mean(best_c0)
    c1mean = np.mean(best_c1)
    c0mean /= args.n
    c1mean /= args.n
    with open("alpha6/alpha5to6", "a") as f:
        f.write("best_mean: {}".format(c0mean))
    with open("alpha6/alpha5", "a") as f:
        f.write("best_mean: ".format(c1mean))
